...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_rows; i++)
    {
        ...
        {
            if (winner_ranks[j] == global_max.rank)
            {
                can_add = 0;
                break;
            }
        ...
        if (can_add)
        {
            winner_ranks[new_size] = global_max.rank;
            new_size++;
        }
    ...
    // found unique winners using winner_ranks array
    // int unique_winners[size];
    // int count = 0;
    // for (int i = 0; i < num_rows; i++)
    // {
    //     int current = winner_ranks[i];
    //     int found = 0;
    //     for (int j = 0; j < count; j++)
    //     {
    //         if (unique_winners[j] == current)
    //         {
    //             found = 1;
    //             break;
    //         }
    ...
    //     if (!found)
    //     {
    //         unique_winners[count] = current;
    //         count++;
    //     }
    // }

    printf("\nProcess %d winner ranks per row:\n", rank);
    print_vector(winner_ranks, new_size);
    fflush(stdout);
    ...
    if (new_comm != MPI_COMM_NULL)
    {
        ...
        MPI_Barrier(new_comm); // sincronizza per stampare ordinato
        for (int p = 0; p < new_comm_size; p++)
        {
            if (new_comm_rank == p)
            {
                printf("\n[GLOBAL RANK %d | NEW_COMM %d/%d] sum_V matrix:\n",
                       rank, new_comm_rank, new_comm_size);
